...
class ZKSNARKService
{
     ...
     *
     * Struct + Groth16 partial verification untuk withdraw circuit.
     * Pairing check final didelegasikan ke kontrak on-chain (`ZKPayment.withdraw`).
     *
     * Public inputs yang diperiksa: commitment, nullifier, recipient, amount.
     * Semua harus ada + dalam batas field bn128.
     *
     * @param string $proof Base64 encoded JSON proof
     * @return bool
     */
    public function verifyWithdrawProof(string $proof): bool
    {
        Log::info('[ZKSNARKService] Verifying withdraw proof...');

        try {
            if ($proof === '') {
                Log::warning('[ZKSNARKService] Empty withdraw proof');
                return false;
            }
            ...
            // Check proof type
            if (($proofData['proofType'] ?? '') !== 'withdraw') {
                Log::error('[ZKSNARKService] Invalid proof type for withdraw');
                return false;
            }

            // Validate required public inputs (sesuai withdraw.circom)
            // commitment, nullifier, recipient, amount
            $publicInputs = $proofData['publicInputs'] ?? [];
            $required = ['commitment', 'nullifier', 'recipient', 'amount'];
            foreach ($required as $field) {
                if (!isset($publicInputs[$field]) || $publicInputs[$field] === '') {
                    Log::error('[ZKSNARKService] Missing withdraw field: ' . $field);
                    return false;
                }
            }

            // Validate fields within bn128 scalar field
            foreach ($required as $field) {
                $value = (string) $publicInputs[$field];
                if (!ctype_digit($value)) {
                    Log::error('[ZKSNARKService] Non-decimal withdraw field: ' . $field);
                    return false;
                }
                if (bccomp($value, self::SNARK_SCALAR_FIELD) >= 0) {
                    Log::error('[ZKSNARKService] Withdraw field exceeds scalar field: ' . $field);
                    return false;
                }
            }

            // Verify Groth16 partial (struct + on-curve check). Pairing check final
            // didelegasikan ke kontrak on-chain — lenient di server (pattern sama
            // dengan verifyTransferProof / verifyBalanceProof).
            if (isset($proofData['publicSignals']) && !empty($proofData['publicSignals'])) {
                $cryptoVerified = $this->verifyGroth16Proof(
                    'withdraw',
                    $proofData['proof'],
                ...
                if (!$cryptoVerified) {
                    Log::warning('[ZKSNARKService] Withdraw Groth16 partial verification failed');
                }
            }

            // Nullifier replay guard (DB-level) — sama seperti transfer proof.
            // Caller (PaymentController) tetap bertanggung-jawab markNullifierUsed
            // setelah on-chain settlement berhasil.
            if (!$this->verifyNullifier($publicInputs['nullifier'])) {
                Log::error('[ZKSNARKService] Withdraw nullifier already used (double-spend attempt)');
                return false;
            }

            Log::info('[ZKSNARKService] ✓ Withdraw proof verified (struct + field check)');
            return true;

        } catch (\Exception $e) {
            Log::error('[ZKSNARKService] Withdraw proof error: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Extract withdraw public inputs untuk PaymentController.
     */
    public function extractWithdrawPublicInputs(string $proof): ?array
    {
        $data = $this->decodeProof($proof);
        if (!$data || ($data['proofType'] ?? '') !== 'withdraw') {
            return null;
        }
        if (!isset($data['publicInputs']) || !is_array($data['publicInputs'])) {
            return null;
        }
        return $data['publicInputs'];
    }

    /**
     * Verify Private Transfer Proof (shielded-keypair).
     *
     * Struct + field check + nullifier replay guard. Pairing check final
     * didelegasikan ke kontrak on-chain (ZKPayment.privateTransfer).
     *
     * Public inputs: senderCommitment, nullifier, newSelfCommitment, recipientCommitment.
     */
    public function verifyTransferProof(string $proof): bool
    {
        Log::info('[ZKSNARKService] Verifying private_transfer proof...');
        try {
            if ($proof === '') {
                return false;
            }
            ...
            if (($proofData['proofType'] ?? '') !== 'private_transfer') {
                Log::error('[ZKSNARKService] Invalid proof type for transfer');
                return false;
            }

            $publicInputs = $proofData['publicInputs'] ?? [];
            $required = ['senderCommitment', 'nullifier', 'newSelfCommitment', 'recipientCommitment'];
            foreach ($required as $field) {
                if (!isset($publicInputs[$field]) || $publicInputs[$field] === '') {
                    Log::error('[ZKSNARKService] Missing transfer field: '.$field);
                    return false;
                }
                $value = (string) $publicInputs[$field];
                if (!ctype_digit($value)) {
                    Log::error('[ZKSNARKService] Non-decimal transfer field: '.$field);
                    return false;
                }
                if (bccomp($value, self::SNARK_SCALAR_FIELD) >= 0) {
                    Log::error('[ZKSNARKService] Transfer field exceeds scalar field: '.$field);
                    return false;
                }
            }

            if (isset($proofData['publicSignals']) && !empty($proofData['publicSignals'])) {
                $this->verifyGroth16Proof('private_transfer', $proofData['proof'], $proofData['publicSignals']);
            }

            if (!$this->verifyNullifier($publicInputs['nullifier'])) {
                Log::error('[ZKSNARKService] Transfer nullifier already used (double-spend attempt)');
                return false;
            }

            Log::info('[ZKSNARKService] ✓ Transfer proof verified (struct + field check)');
            return true;
        } catch (\Exception $e) {
            Log::error('[ZKSNARKService] Transfer proof error: '.$e->getMessage());
            return false;
        }
    }

    /**
     * Extract transfer public inputs untuk PaymentController.
     */
    public function extractTransferPublicInputs(string $proof): ?array
    {
        $data = $this->decodeProof($proof);
        if (!$data || ($data['proofType'] ?? '') !== 'private_transfer') {
            return null;
        }
        if (!isset($data['publicInputs']) || !is_array($data['publicInputs'])) {
            return null;
