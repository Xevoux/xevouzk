...
//  - `private_transfer` (kirim): HANYA receipt_ref opaque + type — tx_hash TIDAK
//    dikirim (turunan senderSalt rahasia: sha256(senderCommitment‖senderSalt)), supaya
//    server tak bisa menautkan akun pengirim ke transaksi on-chain.
...
export async function recordEvent({ type, polygonTxHash, receiptRef, amountMatic, receiverAddress, csrfToken }) {
    const usesReceiptRef = type === 'private_transfer' || type === 'private_receive';
    if (usesReceiptRef) {
    ...
    try {
        // Privat (kirim/terima) → kirim receipt_ref (BUKAN tx_hash). Lainnya → polygon_tx_hash.
        const payload = usesReceiptRef
